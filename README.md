...
A **"2.5D view"** toggle in the canvas toolbar flips the floor between the everyday **top-down editor** and an **isometric presentation** — a "3D-ish", plant-simulation-style render where every rack, dock, conveyor and station is drawn with **height**, as an **extruded iso block** (a top face plus two shaded sides), so a high-bay AS/RS reads as a tower over the low docks and conveyors instead of a flat rectangle.

- **What it is (and is not).** This is an **illustrative visualization for presentation and animation — not a survey and not a BIM model.** Element heights are **illustrative defaults**: where the domain model already carries an assumed `heightM` (the same value `domain.js` feeds the **[IFC export](#export-to-bim-ifc)**) the iso view **reuses that single number** so the two features never drift; none of the heights are measured. The BIM/IFC export remains the separate, structured deliverable — this is just a nicer way to *look* at the layout.
- **Viewing, not editing.** Iso mode is **presentation-only**: placing, selecting and dragging elements are disabled (the element hit-test is top-down-only, which keeps the iso scene simple and honest). You can still **drag to pan, wheel-zoom and Fit** — the iso scene composes with the exact same `view.js` zoom/pan/Fit transform as the top-down view, because it is drawn inside the same world transform. Picking a palette tool while in iso **auto-switches back** to the editable top-down view so the placement lands. **The top-down view stays the accurate, editable source of truth** — toggling back restores full editing, unchanged.
- **How it's drawn.** A pure `WT.iso.project(cx, cy, cz)` maps a world cell + height (metres) to a **classic 2:1 dimetric** offset (`+x` → screen right-and-down, `+y` → left-and-down, `+z` → up); blocks are **depth-sorted back-to-front** (painter's algorithm on the footprint's front corner) and coloured with each element's existing type colour. The live **material-flow MUs and station queues** are projected into the iso scene too, so the animation plays in 2.5D. The accurate top-down aids (**heatmap**, compliance/aisle overlays) are **intentionally skipped** in iso — they belong to the measured top-down view.
- **Tested.** The projection and height/sort logic live in a **pure, DOM-free** module ([`iso.js`](iso.js), `WT.iso`), unit-tested headlessly by `node verify_iso.js` (17th harness): `project()` is deterministic and satisfies the iso invariants (screen directions for `±x/±y`, `+z` raising the point, linearity → collinear-stays-collinear, the 2:1 ratio, a known cell → the expected offset), `elementHeight` is **positive and finite for every element type** and matches the domain `heightM`, the depth sort orders a known set back-to-front (stable, non-mutating), and toggling the view mode is a **no-op on the layout/config** (round-trip unchanged). The canvas renderer (`drawScene`) is verified live in the browser (a real DOM can't run in the headless sandbox).
...
Framed honestly, and non-negotiably: **this is a SYNTHETIC teaching *animation* — NOT a real discrete-event-simulation (DES) engine, NOT a measurement and NOT a certification.** **What the base P3 animation does *not* capture:** collisions and obstacle avoidance are never modelled, and the base routing is transparent straight-segment waypoint routing between zone centroids; the throughput/speed come from the documented `wms.js` order-of-magnitude heuristic (not vendor specs). Station queues, congestion and conveyor-following routing are added as an honest heuristic layer in **P3.2** (below) — still not a DES engine. A SYNTHETIC banner sits at the top of the panel and the same label is embedded in the model output.
...
## Material-flow realism: stations, conveyor routing & queues (P3.2)

P3.2 makes the live flow read more like a real plant sim by adding three things **on top of** `flowsim.js`, without changing its API — the existing straight-line behaviour is preserved as the honest fallback:

- **Pick / put / pack stations as active objects.** The layout's pick faces, staging (put-away) buffers and pack stations become **active servers**. Each station has a **service rate** (units/tick) and its own **FIFO queue**. A station's service rate is taken **directly from the documented `wms.js` stage capacity**, divided across the stations of that stage — so the *aggregate* service rate of a stage equals its `WT.wms` capacity, and throughput stays tied to the operations layer. An MU arriving at a station joins its **shortest queue**, is held there, and is released when the station serves it; when **arrivals exceed the service rate the queue grows** (congestion emerges), exactly as a bottleneck backs up. Units are still conserved: `spawned == in-flight + completed` at every step, where **in-flight now counts queued MUs too**.
- **Conveyor-following routing.** When the layout has **conveyor / track cells that form a connected path** between the storage and picking waypoints, the boxes travel **along the centres of those cells** — a polyline discovered by a **deterministic BFS over adjacent conveyor cells** (nearest entry cell → nearest exit cell, collinear runs simplified), so goods ride the conveyor instead of cutting a straight line. If **no connected conveyor path exists**, routing **falls back to the original straight-segment behaviour** (optionally via a single conveyor/RGV/AGV centroid) — unchanged. It is a greedy cell walk, **not real pathfinding and not collision-aware**.
- **Queues / congestion visualization.** Each station is drawn as a **ring with a live queue-count badge**, coloured by congestion level (green → amber → red against a queue threshold); MUs waiting in a **congested** queue get a red congestion outline. All of it is drawn **inside the same world zoom/pan transform** as the boxes, so it stays glued to the floor. The corner legend and the panel readout surface **max queue** and **congested-station count** for the whole floor.

A documented, opt-in `arrivalUnitsPerHr` what-if lets you drive arrivals **above** the balanced line rate to watch a queue build; by default arrivals follow the `wms.js` bottleneck rate, so **the queues stay balanced and drain to empty** — congestion is an emergent property of an overloaded station, not a permanent artifact.

Framed honestly, and non-negotiably: **the stations are a transparent HEURISTIC — FIFO servers whose service rates are the `wms.js` order-of-magnitude capacities, NOT queueing theory, NOT a real discrete-event-simulation engine, NOT measured and NOT a certification.** The conveyor route is a greedy/BFS cell walk, and the honest fallback is the original straight-segment routing.

Because the drawing lives in the DOM it is not exercised headlessly; the **model** is validated by `node verify_flowB.js` (harness 16 in `node test/run-all.mjs`, 18 checks): pick/put/pack stations identified from a layout that has all three, **determinism including the station queues** on an examples and a generated layout, whole-tick determinism (`step(N) == N·step(1)`) with queues, **unit conservation counting queued MUs at every step**, conveyor-routed waypoints that **lie on conveyor cells** (with the straight-segment fallback when there is no conveyor), a **monotonically growing queue** when arrivals exceed service, queues **draining to empty** at the `wms`-tied rates, station service rates **reconstructing the `WT.wms` capacities**, MUs and queues staying in-bounds, and the SYNTHETIC / not-a-DES / not-a-measurement / not-a-certification labels. The other 15 harnesses (including `verify_flowsim.js`) still pass unchanged.
...
- **C1 — 2.5D isometric view. ✅ Done.** A presentation-only **"2.5D view"** toolbar toggle: `iso.js` (`WT.iso`) draws the floor as an **isometric, plant-simulation-style** scene where every element is an **extruded, depth-sorted iso block** with height, and the live flow MUs/queues are projected into it — all inside the **same `view.js` zoom/pan/Fit transform** as the top-down view. Heights **reuse the domain `heightM`** the IFC export uses (a single, illustrative source), editing stays **top-down-only** (iso is view/animate), and the pure projection/height/sort logic is verified by `verify_iso.js` (16 checks). An **illustrative visualization — not a survey, not a BIM model**; the top-down plan remains the accurate, editable source of truth. See "2.5D isometric view" above.
